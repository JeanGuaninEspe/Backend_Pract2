...
const { Usuario, Sala, Mensaje } = models;

export const getMensajeByUser = async (req, res) => {
  const miId = req.uid;
...
export const getMensajeByRoom = async (req, res) => {
...
export const getMensajeByRoom2 = async (req, res) => {
  const { salaId } = req.params;
  const { limite = 50, desde = 0 } = req.query;
  const miId = req.uid;
